...
<head>
  ...
  <link
    href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:ital,wght@0,300;0,400;0,500;0,600;0,700;0,800;1,400;1,600&family=Barlow+Condensed:ital,wght@0,600;0,700;0,800;0,900;1,600;1,700;1,800;1,900&family=Permanent+Marker&family=Orbitron:wght@700&display=swap"
    rel="stylesheet" />
...
<body class="bg-[#FAF9F6] text-slate-900 font-sans antialiased overflow-x-hidden">
  ...
  <div id="member-overlay"
    ...
    onclick="closeMemberPanel()"></div>

  <!-- Sidebar Tab Trigger (vertical pill on right edge) -->
  <button id="member-tab-btn" onclick="toggleMemberPanel()" aria-label="Daftar Member" class="fixed right-0 top-1/2 -translate-y-1/2 z-[91] flex flex-col items-center justify-center gap-1.5
    ...
    <svg class="w-4 h-4 rotate-180 transition-transform duration-300" id="member-tab-icon" fill="none"
  ...
  <!-- Slide-in Panel -->
  <aside id="member-panel" class="fixed top-0 right-0 h-full w-full max-w-md z-[92] bg-white shadow-2xl shadow-black/20
         translate-x-full transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)]
         flex flex-col overflow-y-auto">

    <!-- Panel Header -->
    <div
      class="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 px-8 pt-10 pb-8 relative overflow-hidden shrink-0">
      <div class="absolute -top-10 -right-10 w-40 h-40 bg-brand-gold/20 rounded-full blur-3xl pointer-events-none">
      </div>
      <!-- Close Button -->
      <button onclick="closeMemberPanel()"
        class="absolute top-5 right-5 w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition"
        aria-label="Tutup">
      ...
      <p class="text-slate-400 text-sm mt-2">Isi form di bawah ini dan tim kami akan segera menghubungi Anda dalam 1×24
        jam.</p>
    </div>

    <!-- Form Body -->
    <style>
      #member-form::-webkit-scrollbar { display: none; }
    </style>
    <form id="member-form" method="POST" action="<?= \yii\helpers\Url::to(['site/submit-join']) ?>"
      class="flex-1 px-8 py-6 flex flex-col gap-4 bg-[#121212] overflow-y-auto" style="scrollbar-width: none; -ms-overflow-style: none;">
      <input type="hidden" name="<?= Yii::$app->request->csrfParam; ?>" value="<?= Yii::$app->request->csrfToken; ?>" />

      <!-- Username -->
      <div class="flex flex-col gap-1.5">
        <label class="text-xs font-bold text-slate-400 uppercase tracking-wider" for="m-username">Username <span class="text-red-500">*</span></label>
        <input id="m-username" name="username" type="text" required placeholder="Masukkan Username" class="w-full !bg-none px-4 py-2.5 rounded-xl border border-slate-700 bg-[#1c1c1c] text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-gold/40 focus:border-brand-gold transition" />
      </div>
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <input id="m-name" name="name" type="text" required placeholder="Sesuai KTP" class="w-full !bg-none px-4 py-2.5 rounded-xl border border-slate-700 bg-[#1c1c1c] text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-gold/40 focus:border-brand-gold transition" />
      </div>
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <div class="flex">
          ...
          <input id="m-phone" name="whatsapp_no" type="tel" required placeholder="812345678" class="flex-1 px-4 py-2.5 rounded-r-xl border border-slate-700 bg-[#1c1c1c] text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-gold/40 focus:border-brand-gold transition" />
        </div>
      </div>
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <input id="m-email" name="email" type="email" required placeholder="nama@email.com" class="w-full !bg-none px-4 py-2.5 rounded-xl border border-slate-700 bg-[#1c1c1c] text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-gold/40 focus:border-brand-gold transition" />
      </div>
      
      <!-- Password -->
      <div class="flex flex-col gap-1.5">
        <label class="text-xs font-bold text-slate-400 uppercase tracking-wider" for="m-password">Password <span class="text-red-500">*</span></label>
        <input id="m-password" name="password" type="password" required minlength="6" placeholder="Minimal 6 karakter" class="w-full !bg-none px-4 py-2.5 rounded-xl border border-slate-700 bg-[#1c1c1c] text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-gold/40 focus:border-brand-gold transition" />
      </div>

      <!-- Gender -->
      <div class="flex flex-col gap-1.5">
        <label class="text-xs font-bold text-slate-400 uppercase tracking-wider" for="m-gender">Gender <span class="text-red-500">*</span></label>
        <select id="m-gender" name="gender" required class="w-full !bg-none px-4 py-2.5 rounded-xl border border-slate-700 bg-[#1c1c1c] text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-gold/40 focus:border-brand-gold transition appearance-none cursor-pointer">
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <select id="m-goal" name="fitness_goal" required class="w-full !bg-none px-4 py-2.5 rounded-xl border border-slate-700 bg-[#1c1c1c] text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-gold/40 focus:border-brand-gold transition appearance-none cursor-pointer">
    ...
    <div id="member-success"
      class="hidden flex-1 flex flex-col items-center justify-center px-8 py-16 text-center bg-[#121212]">
      ...
      <p class="text-slate-400 text-sm mb-8">Tim kami akan segera menghubungi Anda melalui nomor WhatsApp yang terdaftar
        untuk konfirmasi membership.</p>
      <button onclick="closeMemberPanel()"
        class="px-8 py-3 rounded-full bg-brand-gold text-white font-bold text-sm hover:bg-brand-gold-hover transition">
        Kembali ke Halaman
      </button>
    </div>
  </aside>
  ...
  <?php if (Yii::$app->controller->route !== 'site/join' && Yii::$app->controller->route !== 'site/trial' && Yii::$app->controller->route !== 'site/login' && Yii::$app->controller->route !== 'site/settings'): ?>
    ...
    <header class="fixed top-0 left-0 right-0 z-50" data-purpose="site-navigation" id="main-header">
      <!-- Background Layer -->
      <div id="header-bg"
        class="absolute inset-0 bg-[#121316]/95 backdrop-blur-md border-b border-white/10 opacity-0 transition-opacity duration-300 pointer-events-none">
      </div>
      ...
      <div class="relative max-w-7xl mx-auto px-6 sm:px-10 lg:px-16 h-20 flex items-center justify-between z-10">
        ...
        <nav class="hidden md:flex items-center gap-8 text-sm font-medium text-white/80">
          <a class="<?= Yii::$app->controller->action->id === 'membership' ? 'text-brand-gold font-bold' : 'hover:text-brand-gold' ?> transition-colors"
            href="<?= \yii\helpers\Url::to(['site/membership']) ?>">Membership</a>
          <a class="<?= Yii::$app->controller->action->id === 'about' ? 'text-brand-gold font-bold' : 'hover:text-brand-gold' ?> transition-colors"
            href="<?= \yii\helpers\Url::to(['site/about']) ?>">Tentang Kami</a>
          <a class="<?= Yii::$app->controller->action->id === 'trainer' ? 'text-brand-gold font-bold' : 'hover:text-brand-gold' ?> transition-colors"
            href="<?= \yii\helpers\Url::to(['site/trainer']) ?>">Personal Trainer</a>
          <div class="relative group">
            <a class="<?= in_array(Yii::$app->controller->action->id, ['location-batam', 'location-bali']) ? 'text-brand-gold font-bold' : 'hover:text-brand-gold' ?> transition-colors cursor-pointer inline-flex items-center gap-1"
              href="#">Lokasi
            ...
            <div
              class="absolute top-full left-1/2 -translate-x-1/2 pt-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
              <div
                class="bg-[#1a1a1a] border border-slate-700/60 rounded-xl shadow-2xl shadow-black/40 p-1.5 flex items-center gap-1 overflow-hidden">
                <a href="<?= \yii\helpers\Url::to(['site/location-batam']) ?>"
                  class="flex items-center gap-2 px-4 py-2.5 text-sm text-slate-300 hover:text-white hover:bg-slate-800/60 rounded-lg transition-colors whitespace-nowrap">
                  Batam
                </a>
                <a href="<?= \yii\helpers\Url::to(['site/location-bali']) ?>"
                  class="flex items-center gap-2 px-4 py-2.5 text-sm text-slate-300 hover:text-white hover:bg-slate-800/60 rounded-lg transition-colors whitespace-nowrap">
                  Bali
                </a>
              </div>
        ...
        <div class="flex items-center gap-6">
          ...
          <?php if ($isGuest): ?>
            <div id="header-auth-group"
              class="flex items-center gap-5 transition-all duration-300 opacity-0 pointer-events-none -translate-y-2">
              <a class="group relative overflow-hidden px-6 py-2.5 md:px-8 md:py-3 rounded-full bg-brand-gold text-white text-xs font-bold tracking-wide transition-all duration-300 shadow-[0_10px_25px_rgba(212,175,55,0.4)] hover:shadow-[0_15px_30px_rgba(212,175,55,0.6)] hover:-translate-y-0.5 inline-flex items-center justify-center"
                ...
                <span class="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <span
                    class="w-[300px] h-[300px] rounded-full bg-black/20 scale-0 group-hover:scale-100 transition-transform duration-500 ease-out z-0"></span>
                </span>
                <span class="relative z-10 uppercase">Daftar Membership</span>
              </a>
              <a href="<?= \yii\helpers\Url::to(['site/login']) ?>"
                class="text-xs font-bold uppercase tracking-wider text-slate-300 hover:text-white transition-colors">
                Masuk
              </a>
            </div>
          <?php endif; ?>
  ...
  <?php if (strpos(Yii::$app->request->url, '/join') === false && strpos(Yii::$app->request->url, '/trial') === false && strpos(Yii::$app->request->url, '/login') === false && strpos(Yii::$app->request->url, '/settings') === false && strpos(Yii::$app->request->url, '/membership') === false && strpos(Yii::$app->request->url, '/checkout') === false && Yii::$app->controller->route !== 'site/join' && Yii::$app->controller->route !== 'site/trial' && Yii::$app->controller->route !== 'site/login' && Yii::$app->controller->route !== 'site/settings' && Yii::$app->controller->route !== 'site/membership' && Yii::$app->controller->route !== 'site/checkout' && Yii::$app->controller->route !== 'site/location-batam' && Yii::$app->controller->route !== 'site/location-bali' && Yii::$app->controller->route !== 'site/location-detail'): ?>
    ...
    <section class="relative bg-[#0d0f13] overflow-hidden" id="lokasi">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-4 lg:gap-8 max-w-none" style="height: 700px;">
          ...
          <div class="lg:col-span-3 flex flex-col gap-4 pl-0 sm:pl-2">
            ...
            <div class="flex gap-4 w-full border-b border-white/10 pb-2 mb-2">
              <button onclick="switchRegion('batam')" id="region-btn-batam"
                class="region-btn pb-2 border-b-2 border-brand-gold text-brand-gold font-bold transition-all">Wilayah
                Batam</button>
              <button onclick="switchRegion('bali')" id="region-btn-bali"
                class="region-btn pb-2 border-b-2 border-transparent text-slate-400 font-bold hover:text-white transition-all">Wilayah
                Bali</button>
            </div>
            ...
            <div class="flex flex-wrap gap-3 w-full" id="branch-buttons-container">
              <!-- Batam Branches -->
              <button onclick="flyToBranch(0)" id="branch-btn-0" data-region="batam"
                class="branch-btn flex-1 min-w-[140px] py-3 px-4 rounded-xl transition-all duration-300 border-2 border-brand-gold bg-brand-gold/10 flex items-center justify-center gap-3">
                <div
                  class="w-8 h-8 rounded-full bg-brand-gold/20 flex items-center justify-center shrink-0 hidden sm:flex">
              ...
              <button onclick="flyToBranch(1)" id="branch-btn-1" data-region="batam"
                class="branch-btn flex-1 min-w-[140px] py-3 px-4 rounded-xl transition-all duration-300 border-2 border-white/10 hover:border-white/30 flex items-center justify-center gap-3">
              ...
              <button onclick="flyToBranch(2)" id="branch-btn-2" data-region="bali"
                class="branch-btn flex-1 min-w-[140px] py-3 px-4 rounded-xl transition-all duration-300 border-2 border-white/10 hover:border-white/30 items-center justify-center gap-3 hidden">
                <div class="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center shrink-0 hidden sm:flex">
                  <i class="fas fa-dumbbell text-slate-400 text-xs"></i>
                </div>
                <div class="text-left">
              ...
              <button onclick="flyToBranch(3)" id="branch-btn-3" data-region="bali"
                class="branch-btn flex-1 min-w-[140px] py-3 px-4 rounded-xl transition-all duration-300 border-2 border-white/10 hover:border-white/30 items-center justify-center gap-3 hidden">
                <div class="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center shrink-0 hidden sm:flex">
                  <i class="fas fa-dumbbell text-slate-400 text-xs"></i>
                </div>
          ...
          <div class="lg:col-span-2 overflow-hidden relative h-full">
            <div
              class="absolute inset-x-0 top-0 h-24 bg-gradient-to-b from-[#0d0f13] to-transparent z-10 pointer-events-none">
            </div>
            <div
              class="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-[#0d0f13] to-transparent z-10 pointer-events-none">
            </div>

            <!-- Scrollable Container -->
            <div class="h-full w-full overflow-y-auto overflow-x-hidden scrollbar-hide" id="gallery-scroll-container"
      ...
      <style>
        ...
        .hercules-popup .leaflet-popup-content-wrapper {
          border-radius: 12px;
          box-shadow: 0 10px 30px rgba(0, 0, 0, 0.3);
        }

        .hercules-popup .leaflet-popup-tip {
  ...
  <?php if (!isset($this->params['hideFooter']) || !$this->params['hideFooter']): ?>
    <footer class="bg-white border-t border-slate-200 pt-16 pb-0 relative overflow-hidden" data-purpose="page-footer">
      <div class="max-w-7xl mx-auto px-6">
        ...
        <div
          ...
          <div class="w-full lg:w-auto">
            ...
            <div class="flex flex-col sm:flex-row gap-2">
              <input
                class="px-4 py-2.5 rounded-xl border border-slate-300 text-xs focus:ring-brand-gold focus:border-brand-gold w-full sm:w-64"
                placeholder="Masukkan email Anda" type="email" />
              <button
                class="px-6 py-2.5 rounded-xl bg-brand-gold text-white text-xs font-bold hover:bg-brand-gold-hover transition"
                type="button">
                Berlangganan
              </button>
            </div>
            <p class="text-[11px] text-slate-400 mt-2">Dengan mendaftar, Anda menyetujui <a
                class="underline hover:text-slate-600 transition" href="javascript:void(0)">Kebijakan Privasi</a> kami</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 mb-16">
          ...
          <div class="lg:col-span-2">
            <a class="flex items-center gap-3 mb-4" href="#">
              <img src="/img/hercules.jpeg" alt="Hercules Fitness Logo"
                class="w-10 h-10 rounded-full object-cover shadow-lg shadow-brand-gold/30">
              <div class="flex flex-col leading-none">
            ...
            <p class="text-xs text-slate-500 leading-relaxed max-w-sm mb-4">
              Pusat kebugaran terbaik di Batam dan Bali. Kami hadir untuk membantu Anda mencapai target fisik dengan
              program latihan profesional dan tenaga ahli bersertifikat.
            </p>
            ...
            <div class="flex items-center gap-3 text-slate-400">
              <a class="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center hover:text-brand-gold transition"
                href="https://instagram.com/hercules.fitnesscentre" target="_blank" title="Instagram">
                <i class="fab fa-instagram text-sm"></i>
              </a>
              <a class="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center hover:text-brand-gold transition"
                href="https://wa.me/6282286680539" target="_blank" title="WhatsApp">
                <i class="fab fa-whatsapp text-sm"></i>
              </a>
              <a class="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center hover:text-brand-gold transition"
                href="https://maps.app.goo.gl/dghCRwrLjwhYoE4H8" target="_blank" title="Google Maps">
          ...
          <div>
            ...
            <ul class="space-y-2.5 text-xs text-slate-600">
              <li><a class="hover:text-brand-gold transition"
                  href="<?= \yii\helpers\Url::to(['site/index']) ?>">Beranda</a></li>
              <li><a class="hover:text-brand-gold transition"
                  href="<?= \yii\helpers\Url::to(['site/membership']) ?>">Harga Membership</a></li>
              <li><a class="hover:text-brand-gold transition"
                  href="<?= \yii\helpers\Url::to(['site/location-batam']) ?>">Lokasi Batam</a></li>
              <li><a class="hover:text-brand-gold transition"
                  href="<?= \yii\helpers\Url::to(['site/location-bali']) ?>">Lokasi Bali</a></li>
              <li><a class="hover:text-brand-gold transition"
                  href="<?= \yii\helpers\Url::to(['site/trainer']) ?>">Personal Trainer</a></li>
              <li><a class="hover:text-brand-gold transition" href="<?= \yii\helpers\Url::to(['site/join']) ?>">Daftar
                  Member</a></li>
              <li><a class="hover:text-brand-gold transition" href="<?= \yii\helpers\Url::to(['site/login']) ?>">Masuk
                  Akun</a></li>
            </ul>
          </div>
          <!-- Col 3: About -->
          <div>
            ...
            <ul class="space-y-2.5 text-xs text-slate-600">
              <li><a class="hover:text-brand-gold transition" href="<?= \yii\helpers\Url::to(['site/about']) ?>">Profil
                  Hercules Fitness</a></li>
              <li><a class="hover:text-brand-gold transition"
                  href="<?= \yii\helpers\Url::to(['site/about', '#' => 'filosofi']) ?>">Kisah & Visi Misi</a></li>
              <li><a class="hover:text-brand-gold transition"
                  href="<?= \yii\helpers\Url::to(['site/about', '#' => 'professional-team']) ?>">Tim Profesional</a></li>
              <li><a class="hover:text-brand-gold transition"
                  href="<?= \yii\helpers\Url::to(['site/about', '#' => 'ulasan-google']) ?>">Ulasan Member (4.9★)</a></li>
              <li><a class="hover:text-brand-gold transition" href="<?= \yii\helpers\Url::to(['site/contact']) ?>">Hubungi
                  Kami</a></li>
            </ul>
          </div>
          <!-- Col 4: Contact -->
          <div>
            ...
            <ul class="space-y-2.5 text-xs text-slate-600">
              <li class="flex items-start gap-1.5"><span class="text-brand-gold mt-0.5">📍</span> <span
                  class="font-medium">Batam:</span> Batu Ampar, Batu Besar, MTC</li>
              <li class="flex items-start gap-1.5"><span class="text-brand-gold mt-0.5">📍</span> <span
                  class="font-medium">Bali:</span> Canggu, Kuta</li>
              <li class="flex items-start gap-1.5"><span class="text-brand-gold mt-0.5">📞</span> <a
                  href="https://wa.me/6282286680539" class="hover:text-brand-gold transition" target="_blank">+62
                  822-8668-0539</a></li>
              <li class="flex items-start gap-1.5"><span class="text-brand-gold mt-0.5">📷</span> <a
                  href="https://instagram.com/hercules.fitnesscentre" class="hover:text-brand-gold transition"
                  target="_blank">@hercules.fitnesscentre</a></li>
              <li class="flex items-start gap-1.5"><span class="text-brand-gold mt-0.5">🕐</span> Sen–Jum: 07.00–23.00
              </li>
              <li class="flex items-start gap-1.5"><span class="text-brand-gold mt-0.5">🕐</span> Sab–Min: 07.00–22.00
              </li>
            </ul>
          </div>
        </div>
        <!-- Copyright Bar -->
        <div
          class="pt-8 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-400 gap-4 mb-4 sm:mb-6">
        ...
        <div
          class="w-full flex justify-center items-end pointer-events-none select-none -mb-6 sm:-mb-9 md:-mb-12 lg:-mb-15 mt-6 sm:mt-10 pt-4 overflow-visible">
          <span
            class="font-logo-main text-[13vw] sm:text-[14.5vw] lg:text-[16vw] leading-none text-slate-200/85 tracking-wider uppercase text-center block whitespace-nowrap">
            HERCULES
          </span>
        </div>
    </footer>
  ...
  <div id="program-info-modal"
    ...
    <div
      class="relative bg-white rounded-xl shadow-2xl w-[90%] max-w-4xl flex flex-col md:flex-row overflow-hidden transform scale-95 transition-transform duration-300"
      id="program-modal-content">

      <!-- Close button -->
      <button onclick="closeProgramModal()"
        class="absolute top-4 right-4 z-10 w-8 h-8 rounded-full bg-black/20 hover:bg-black/60 text-white flex items-center justify-center transition-colors backdrop-blur-md">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center order-2 md:order-1">
        ...
        <ul class="text-sm text-slate-700 space-y-3 mb-8 bg-slate-50 p-4 rounded-xl border border-slate-100">
          ...
          <li class="flex items-start gap-3">
            ...
            <div><strong class="text-slate-900">Perlengkapan:</strong> Pakaian olahraga nyaman & membawa botol minum.
            </div>
          </li>
          <li class="flex items-start gap-3">
            <i class="fa-solid fa-bolt text-brand-gold mt-0.5"></i>
            <div><strong class="text-slate-900">Manfaat Utama:</strong> Meningkatkan kekuatan inti tubuh & membakar
              lemak.</div>
          </li>
        </ul>

        <a href="#paket" onclick="closeProgramModal()"
          class="inline-flex w-max items-center justify-center px-8 py-3.5 bg-brand-gold hover:bg-brand-gold-hover text-white rounded-full font-bold transition-colors shadow-lg shadow-brand-gold/30">
          JOIN PROGRAM
        </a>
      </div>
  ...
  <a id="wa-button"
    href="https://wa.me/6282286680539?text=Halo%20Hercules%20Fitness,%20saya%20tertarik%20dengan%20program%20membership"
