...
$variantsJson = isset($variants) ? Json::encode($variants) : '[]';
?>
...
<div class="d-flex flex-column scroll-y px-5 px-lg-10" id="modal_form_product_scroll" data-kt-scroll="false" data-kt-scroll-activate="true" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#modal_form_product_header" data-kt-scroll-wrappers="#modal_form_product_scroll" data-kt-scroll-offset="300px">
    ...
    <div class="row">
        <div class="mb-7 col-lg-2 col-md-2 col-sm-2">
            ...
            <?= $form->field($model, 'produkkode',  ['errorOptions' => ['class' => 'text-danger mt-1']])->textInput(['class' => 'form-control form-control-solid', 'readonly' => true])->label(false) ?>
        </div>

        <div class="mb-7 col-lg-4 col-md-4 col-sm-4">
            <label class="fw-semibold fs-6 mb-2">Product Name</label>
            <?= $form->field($model, 'namaproduk',  ['errorOptions' => ['class' => 'text-danger mt-1']])->textInput(['class' => 'form-control', 'placeholder' => 'Product Name', 'id' => 'namaproduk', 'required' => true])->label(false) ?>
            <div class="invalid-feedback" id="namaproduk-feedback">Product name is required</div>
        </div>
    ...
    <div class="row">
        ...
        <div id="price-container" class="mb-7 col-lg-4 col-md-4 col-sm-4 col-xs-12">
            <label class="fw-semibold fs-6 mb-2">Price</label>
            <div class="d-flex">
                <span class="bg-primary text-white px-3 py-2 fw-bold d-flex align-items-center"
                    style="border-radius: 6px 0 0 6px; height : 42px;">
                    RP
                </span>
...
<script type="text/javascript">
    ...
    var existingVariants = <?= $variantsJson ?>;
    ...
    function initializeExistingVariants(existingVariants) {
        // Clear any default variants first
        $('.variant-list').empty();
        // toggleImageInput()
        // togglePriceInput();
        ...
        if (!existingVariants || existingVariants.length === 0) {
            // Add a default empty variant if needed
        ...
        // Automatically check the toggle to show variants
        $('#toggleVariantBtn').prop('checked', true);
        $('#varianContainer').show();

        // Add each existing variant
        existingVariants.forEach((variant, index) => {
            const newVariant = `
<div class="variant-item card mb-5 shadow-sm">
    <div class="card-body p-5">
        <div class="d-flex justify-content-between mb-4">
            <h4 class="card-title fw-bold variant-title">Variant #${index + 1}</h4>
        ...
        <div class="row g-3">
            ...
            <input type="hidden" name="variants[${index}][varianid]" value="${variant.varianid || ''}" />

            <!-- Image Container with Dynamic ID -->
            <div class="mb-7 col-lg-12 col-md-12 col-sm-12" id="image-container-${index}">
                <div class="image-input image-input-outline" id="kt_image_${index}" data-kt-image-input="true"
                    style="background-image: url('https://img1.wsimg.com/isteam/ip/3b5c6067-4366-46be-bbaa-75aff3a98515/placeholder.jpg');">

                    <div class="image-input-wrapper"
                        style="background-image: url('${variant.produkfoto ? variant.produkfoto : 'https://img1.wsimg.com/isteam/ip/3b5c6067-4366-46be-bbaa-75aff3a98515/placeholder.jpg'}');">
                    </div>
                    ...
                    <label class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow"
                        data-kt-image-input-action="change" data-toggle="tooltip" title="Change avatar">
                        <i class="fa fa-pen icon-sm text-muted"></i>
                        <input type="file" name="variants[${index}][profile_avatar]" accept=".png, .jpg, .jpeg" />
                        <input type="hidden" name="variants[${index}][avatar_remove]" value="0" />
                    </label>
                    ...
                    <span class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-danger"
                        data-kt-image-input-action="remove" data-toggle="tooltip" title="Remove avatar"
                        style="${variant.produkfoto ? 'display: flex;' : 'display: none;'}">
                        <i class="fa fa-trash text-white"></i>
                    </span>
                </div>
            </div>
            ...
            <div class="col-6 mb-4">
                ...
                <div class="position-relative">
                    ...
                    <input type="text" name="variants[${index}][hargabeli]" class="form-control varian-harga-beli ps-5 text-end"
                           placeholder="Harga Beli" required autocomplete="off" value="${formatNumber(variant.hargabeli || 0)}"
            ...
            <div class="col-6 mb-4">
                ...
                <div class="position-relative">
                    ...
                    <input type="text" name="variants[${index}][hargajual]" class="form-control varian-harga-jual ps-5 text-end"
                           placeholder="Harga Jual" required autocomplete="off" value="${formatNumber(variant.hargajual || 0)}"
                           style="border-radius: 6px; padding-left: 50px;" />
                </div>
            ...
            <div class="col-12 mb-4">
                ...
                <div class="position-relative">
                    ...
                    <input type="text" name="variants[${index}][hargagrosir]" class="form-control varian-harga-grosir ps-5 text-end"
                            placeholder="Harga Grosir" required autocomplete="off" value="${formatNumber(variant.hargagrosir || 0)}"
                            style="border-radius: 6px; padding-left: 50px;" />
                </div>
                </div>
            ...
            <div class="col-md-6 col-lg-4">
                ...
                <input type="text" name="variants[${index}][sku]" class="form-control variant-sku"
                       placeholder="Enter SKU" value="${variant.sku || ''}" />
            </div>

            <div class="col-md-6 col-lg-8">
                <label class="fw-semibold fs-6 mb-2">Deskripsi</label>
                <input type="text" name="variants[${index}][deskripsi]" class="form-control"
                       placeholder="Deskripsi" value="${variant.deskripsi || ''}" />
            </div>

            <!-- Stok - hidden by default as specified in your template -->
            <div class="col-md-6 col-lg-3" style="display: none">
                ...
                <input type="number" name="variants[${index}][stok]" class="form-control"
                       placeholder="Enter Stock" min="0" value="${variant.stok || 0}" />
            </div>
        </div>
    </div>
</div>
`;
            ...
            initVariantPriceInput(index, 'hargabeli');
            initVariantPriceInput(index, 'hargajual');
            initVariantPriceInput(index, 'hargagrosir');

            // Initialize the image handler for this variant
            initVariantImageHandler(index);
        });

        // Update the visibility of price inputs based on variants
        togglePriceInput();
        toggleImageInput();
...
`;
    ...
    function setupFormHandlers() {
        ...
        $('#FormValid').on('submit', function(e) {
            ...
            if (($('#price').val().trim() === '' || $('#price').val().trim() === '0') && $('.variant-item').length < 1) {
    ...
    $(document).ready(function() {
        // var_dump(existingVariants);
        // die;
        setupVariantFunctionality();

        // Check if existing variants data is available (this will be set in your view)
        if (existingVariants.length > 0) {
            initializeExistingVariants(existingVariants);
        }
        ...
        // setupVariantFunctionality();
        setupFormHandlers();
        $('.field-produk-produkkode , .field-deskripsi , .field-produksku, .field-price, .field-namaproduk').removeClass('required');
