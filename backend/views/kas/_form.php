...
<script>
    ...
    var isEditMode = <?= !$model->isNewRecord ? 'true' : 'false' ?>;
    ...
    var detailManager = {
        ...
        addInvoiceDetails: function(invoiceId, invoiceNo, invoiceDetails, contactId) {
            ...
            // Jika dalam mode edit DAN sudah ada initialDetailsData, gunakan data existing
            if (isEditMode && initialDetailsData && initialDetailsData.length > 0) {
            ...
            invoiceDetails.forEach((detail, index) => {
                ...
                $row.find('.detail-description').val(`${detail.deskripsi_produk || 'Produk'} - ${invoiceNo}`);
        ...
        loadExistingData: function() {
            if (initialDetailsData && initialDetailsData.length > 0) {
                ...
                initialDetailsData.forEach((detail, index) => {
                    ...
                    // Set nilai setelah select2 siap (gunakan setTimeout)
                    setTimeout(() => {
                        if (detail.akun) {
                            $row.find('.detail-akun').empty().append(
                                new Option(detail.akun.coa_name_id, detail.akun.coa_id, true, true)
                            ).trigger('change');
                        }
    ...
    $(document).ready(function() {
        ...
        function setupEditMode() {
            // Jika ada initialDetailsData (data existing dari database), prioritaskan itu
            if (initialDetailsData && initialDetailsData.length > 0) {
                setTimeout(function() {
                    detailManager.loadExistingData();
                }, 300);
            }
            // Jika tidak ada data existing tapi ada invoice data, gunakan invoice data
