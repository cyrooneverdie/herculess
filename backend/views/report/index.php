<?php
$title = Yii::$app->lang->t('report', 'report1');
$this->title = $title;
...
<div class="row g-5 gx-xl-10 mb-5 mb-xl-10 mt-5">
	<div class="d-flex gap-5 flex-column flex-column-fluid">
		...
		<div class="d-flex flex-column flex-lg-row gap-5 justify-content-start">
				...
				<div class="card-body">
					<ul class="list-unstyled report-menu">
						<li class="mb-3" data-id="1">
							<button type="button" class="btn btn-light btn-custom-report"><i class="fa-regular fa-star"></i></button>
			...
			<div class="card w-100 w-lg-50 h-350px">
				<div class="card-header border-0 pt-6 text-center">
					<div class="card-title">
		...
		<div class="d-flex flex-column flex-lg-row gap-5 justify-content-start">
			...
			<div class="card w-100 w-lg-50 h-600px">
				<div class="card-header border-0 pt-6 text-center">
					<div class="card-title">
		...
		<div class="d-flex flex-column flex-lg-row gap-5 justify-content-start">
			...
			<div class="card w-100 w-lg-50 h-300px">
				<div class="card-header border-0 pt-6 text-center">
					<div class="card-title">
		...
		<div class="d-flex flex-column flex-lg-row gap-5 justify-content-start">
			...
			<div class="card w-100 w-lg-50 h-200px">
				<div class="card-header border-0 pt-6 text-center">
					<div class="card-title">
...
<script>
    ...
    const favoriteContainer = $('#favorite ul'); // Target elemen <ul> di dalam div #favorite
    const favoriteCard = $('#favorite-card'); // Target card favorit secara keseluruhan
    ...
    function renderFavorites() {
        ...
        if (favorites.length > 0) {
            favoriteCard.show(); // Tampilkan card jika ada favorit
            favorites.forEach(item => {
                let listItem = $(`[data-id="${item.id}"]`).clone(); // Kloning elemen <li> berdasarkan ID
                listItem.find('i').removeClass('fa-regular').addClass('fa-solid'); // Ubah ikon jadi solid
                favoriteContainer.append(listItem); // Tambahkan ke daftar favorit
                $(`[data-id="${item.id}"]`).find('i').removeClass('fa-regular').addClass('fa-solid'); // Perbarui ikon di daftar asli
            });
        } else {
            favoriteCard.hide(); // Sembunyikan card jika tidak ada favorit
        }
    }

    renderFavorites(); // Render favorit saat halaman dimuat

    // Event handler untuk tombol favorit di daftar utama
    $('.report-menu').on('click', '.btn-custom-report', function() {
        let button = $(this);
    ...
    // Event handler untuk tombol favorit di favorite-card
    favoriteContainer.on('click', '.btn-custom-report', function() {
        let button = $(this);
        let listItem = button.closest('li'); // Ambil parent <li>
        let itemId = listItem.data('id'); // ID unik untuk tiap item
        ...
        // Perbarui ikon di daftar utama menjadi fa-regular
        $(`.report-menu [data-id="${itemId}"]`).find('i').removeClass('fa-solid').addClass('fa-regular');
