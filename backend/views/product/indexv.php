<?php
$title = Yii::$app->lang->t('extrasidebar', 'extrasidebar326');
$this->title = $title;
		...
		<table class="table table-row-bordered fs-6 gy-5" id="datatable">
			<thead>
				<tr
					...
					<th class="text-start min-w-130px"><?= Yii::$app->lang->t('produk_table', 'produk_gambar') ?>
					</th>
					<th class="text-start min-w-125px"><?= Yii::$app->lang->t('enum', 'code') ?></th>
					<th class="text-start min-w-250px"><?= Yii::$app->lang->t('tax', 'label1') ?></th>
					<th class="text-start min-w-100px"><?= Yii::$app->lang->t('produk_table', 'produk_kategori') ?>
					</th>
					<th class="text-start min-w-150px"><?= Yii::$app->lang->t('subcategory', 'label1') ?></th>
					<th class="text-start min-w-100px"><?= Yii::$app->lang->t('produk_table', 'produk_jenis') ?>
					</th>
					<th class="text-start min-w-100px"><?= Yii::$app->lang->t('produk_table', 'produk_brand') ?>
					</th>
					<th class="text-start min-w-100px"><?= Yii::$app->lang->t('produk', 'produk_display') ?></th>
					<th class="text-start min-w-100px"><?= Yii::$app->lang->t('series', 'label1') ?></th>
					<th class="text-center min-w-80px"><?= Yii::$app->lang->t('produk', 'produk_unit') ?></th>
					<th class="text-start min-w-250px"><?= Yii::$app->lang->t('produk', 'produk_deskripsi') ?></th>
					<th class="text-start min-w-80px"><?= Yii::$app->lang->t('produk_table', 'produk_status') ?>
					</th>
				</tr>
			</thead>
	...
	<div class="modal-dialog modal-lg-down modal-xl">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title"><?= Yii::$app->lang->t('extra', 'extra38') ?></h5>
			...
			$("#datatable").DataTable({
				...
				columns: [
					...
					},
					{
						...
						className: "text-center align-middle",
						render: function (data, type, row) {
							const updateUrl = "<?= Url::to(['update']) ?>?id=" + row.productid + "&viewType=indexv";
							...
							<div class="dropdown text-center dropend">
								...
								<ul class="dropdown-menu">
									<li>
										<a href="<?= Url::to(['product/detail']) ?>?id=${row.productid}&viewType=indexv" class="dropdown-item text-hover-primary">
...

					{
						...
						render: function (data, type, row) {
							let imgSrc = "<?= Yii::getAlias('@web') ?>/uploads/produk/" + data;
							return `<img src="${imgSrc}" alt="${row.productname}" class="w-100px h-100px border border-gray-300 rounded object-fit-contain">`;
						},
						orderable: false
					},
					{
						...
						render: function (data, type, row) {
							...
							<div class="d-flex flex-column align-items-left mx-auto my-auto w-100">
								<span class="text text-start">${row.productcode || '-'}</span>
					...
					{ data: "categoryname", className: "text-start align-middle" },
					{ data: "subcategoryname", className: "text-start align-middle" },
					{ data: "typename", className: "text-start align-middle" },
					{ data: "brandname", className: "text-start align-middle" },
					{ data: "specname", className: "text-start align-middle" },
					{ data: "series", className: "text-start align-middle" },
					...
					},
					{
						...
						render: function (data) {
							return data == 1 ?
								`<span class="badge badge-success">${translate5}</span>` :
								`<span class="badge badge-danger">${translate6}</span>`;
						}
					}
				],
			...
			$(document).on('click', '.btn-add-variant', function (e) {
				...
				$.ajax({
					...
					success: function (data) {
						let parser = new DOMParser();
						let doc = parser.parseFromString(data, 'text/html');
						$('#modal-content-variant').html(doc.body.innerHTML);
						$('#modal_form_variant').modal('show');

						if (productId) {
							let selectElement = $('#modal-content-variant').find('.product-select');

							$.ajax({
								url: "<?= Url::to(['product/select']) ?>",
								type: 'POST',
								data: { search: '', id: productId, for: 'select2' },
								success: function (res) {
									if (res.items && res.items.length > 0) {
										let product = res.items.find(item => item.id == productId) || res.items[0];

										let newOption = new Option(product.text, product.id, true, true);
					...
					error: function () {
						$('#modal-content-variant').html('<p>Error loading form.</p>');
					}
				});
			});
					...
					if (result.isConfirmed) {
						$.ajax({
							...
							success: function (response) {
								...
								$(`#sub-table-${parentId}`).DataTable().ajax.reload();
								Swal.fire({
									title: "Terhapus!",
