...
if ("$module/$type" == "purchase/delivery" && $type_id == 1) {
    $urladd = Url::to(['createdeli', 'module' => $module, 'type' => $type, 'type_id' => $type_id]);
    $this->title = Yii::$app->lang->t('extrasidebar', 'extrasidebar10');
    $formatnumber = "I";

} elseif ("$module/$type" == "purchase/delivery" && $type_id == 2) {
    $urladd = Url::to(['createdeli', 'module' => $module, 'type' => $type, 'type_id' => $type_id]);
    $this->title = Yii::$app->lang->t('extrasidebar', 'extrasidebar123');
    $formatnumber = "I";
}
        ...
        <table class="table align-left table-row-dashed fs-6 gy-5" id="datatable">
            <thead>
                <tr class="text-start bg-gray-100 fs-6 text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                    ...
                    <th class="text-center min-w-150px">Status Stock In</th>
                    <?php if ($type_id == 1): ?>
                        <th class="text-center min-w-150px">Status Proyek</th>
                    <?php endif; ?>
                    <th class="text-center min-w-150px">Status</th>
                    <th class="text-start min-w-150px"><?= Yii::$app->lang->t('tran', 'tran_no') ?></th>
                    <th class="text-start min-w-150px">Reference No</th>
...
<script>
    $(document).ready(function () {
        ...
        $("#select-all").on("click", function () {
            ...
            $("tbody .select-checkbox:not(:disabled)").each(function () {
                if (isChecked) {
                    $(this)
                        .prop("checked", true)
        ...
        $("#datatable tbody").on("change", ".select-checkbox", function () {
            const totalEnabled = $(".select-checkbox:not(:disabled)").length;
            const totalChecked = $(".select-checkbox:checked").length;

            $("#select-all").prop(
                "checked",
                totalEnabled === totalChecked && totalEnabled > 0
            );
        ...
        function performMassAction(action, statusCode) {
            let selectedIds = $(".select-checkbox:checked:not(:disabled)")
                .map(function () {
                    return $(this).val();
                })
                .get();
        ...
        function addNestedDropdown() {
            $(document).on('click', '.neo-orba', function (e) {
                ...
                var refType = button.data('reftype');
                ...
                if (refType === 'purchase/order' || refType === 'purchase/request') {
                    subTableHTML = `
                        <tr class="expanded-row">
                            <td colspan="11">
                                <div class="m-3 row">
                                    <div class="col-md-8 border-end border-gray-300 pe-3">
                                        <table class="table align-middle table-row-dashed fs-6 gy-5 sub-datatable" id="sub-table-po-${parentId}">
                                            <thead>
                                                <tr class="text-gray-500 bg-gray-100 border-bottom border-gray-200 fs-7 text-uppercase">
                                                    <th class="text-start p-5">Product</th>
                    ...
                    subTableHTML = `
                        <tr class="expanded-row">
                            <td colspan="11">
                                <div class="m-3 row">
                                    <div class="col-md-6 border-end border-gray-300 pe-3">
                                        <table class="table align-middle table-row-dashed fs-6 gy-5 sub-datatable" id="sub-table-return-${parentId}">
                                            <thead>
                                                <tr class="text-gray-500 bg-gray-100 border-bottom border-gray-200 fs-7 text-uppercase">
                                                    <th class="text-start">Product</th>
                                                    <th class="text-center">Return Milik</th>
                                                    <th class="text-center">Return Pinjam</th>
                                    ...
                                    <div class="col-md-6 ps-3">
                                        <div class="d-flex align-items-center gap-2 mb-4">
                                            <span class="badge bg-warning rounded-circle d-flex align-items-center justify-content-center"
                                                ...
                                                <i class="fa-solid fa-triangle-exclamation text-white fs-8"></i>
                                            </span>
                                            <h6 class="fw-bold mb-0 text-gray-700 fs-7 text-uppercase ls-1">Status Penarikan</h6>
                                        </div>
                                        <div class="return-warning" data-id="${parentId}">
                                            <div class="d-flex align-items-center justify-content-center py-5 text-muted fs-7">
                ...
                if (refType === 'purchase/order' || refType === 'purchase/request') {
                    $(`#sub-table-po-${parentId}`).DataTable({
                        processing: true,
                        serverSide: true,
                        ...
                        columns: [
                            { data: "productname", className: "text-start", render: function (data) { return data || '-'; } },
                            { data: "qty", render: function (data) { return data || '0'; } },
                            { data: "received", render: function (data) { return data || '0'; } },
                            {
                                data: null,
                                render: function (data, type, row) {
                                    return `<a class="dropdown-item text-hover-primary btn-variant-in" 
                                        href="<?= Url::to(['variant/variantdeli']) ?>?refid=${row.trandetailid}" 
                                        data-bs-toggle="modal" data-bs-target="#modal_list_track"><i class="fa-solid fa-list"></i></a>`;
                                }
                            }
                        ],
                        order: [],
                    ...
                    $(`#sub-table-return-${parentId}`).DataTable({
                        processing: true,
                        serverSide: true,
                        lengthMenu: [5, 10, 25, 50],
                        pageLength: 5,
                        ...
                        drawCallback: function (settings) {
                            ...
                            var warningBox = $(`.return-warning[data-id="${parentId}"]`);

                            let notReturn = [];

                            data.each(function (value, index) {
                                let totalScan = parseFloat(value.total_scan || 0);
                                let totalHarusReturn = parseFloat(value.total_return || 0);
                                let status = value.statuspro;

                                if (totalScan < totalHarusReturn && status != 15) {

                                    let sisa = totalHarusReturn - totalScan;
                                    let itemHtml = `
                                    <div class="d-flex align-items-start gap-3 p-3 rounded-2 bg-light-warning border border-warning-subtle">
                                        <div class="flex-shrink-0 mt-1">
                                            <span class="badge bg-warning rounded-circle d-flex align-items-center justify-content-center"
                                                style="width:32px;height:32px;">
                                                <i class="fa-solid fa-box text-white fs-8"></i>
                                            </span>
                                        </div>
                                        <div class="flex-grow-1">
                                            <div class="fw-bold text-gray-800 mb-1">${value.productname}</div>
                                            <div class="d-flex flex-wrap gap-2 align-items-center">
                                                <span class="badge badge-light-secondary text-gray-600 d-flex align-items-center gap-1 py-1 px-2 fs-8">
                                                    <i class="fa-solid fa-barcode fs-9"></i>
                                                    ${value.pending_barcodes || '-'}
                                                </span>
                                                <span class="badge bg-danger py-1 px-2 fs-8 text-white d-flex align-items-center gap-1">
                                                    <i class="fa-solid fa-circle-minus me-1 fs-9 text-white"></i>Kurang ${sisa} item
                                                </span>
                                            </div>
                                        </div>
                                    </div>`;
                                    notReturn.push(itemHtml);
                                }
                            });

                            if (notReturn.length > 0) {
                                warningBox.html(`
                                    <div class="card card-flush border border-warning-subtle">
                                ...
                                warningBox.html(`
                                <div class="card card-flush border border-success-subtle h-100">
                                    <div class="card-body d-flex flex-column align-items-center justify-content-center text-center py-5 gap-3">
                                        <div class="d-flex align-items-center justify-content-center rounded-circle bg-light-success"
                                            style="width:56px;height:56px;">
                                            <i class="fa-solid fa-check-double text-success fs-2"></i>
                                        </div>
                                        <div>
                                            <div class="fw-bold text-gray-800 fs-6">Semua Barang Sudah Di Scan</div>
                                        </div>
                                    </div>
                                </div>
                            `);
                        ...
                        columns: [
                            { data: "productname", className: "text-start", render: function (data) { return data || '-'; } },
                            { data: "return", render: function (data) { return data || '0'; } },
                            { data: "return2", render: function (data) { return data || '0'; } },
                            {
                                data: null,
                                render: function (data, type, row) {
                                    let r1 = parseFloat(row.return || 0);
                                    let r2 = parseFloat(row.return2 || 0);
                                    return r1 + r2;
                                }
                            },
                            {
                                data: null,
                                render: function (data, type, row) {
                                    return `<a class="dropdown-item text-hover-primary btn-variant-deli-rtn" href="<?= Url::to(['variant/variantdeli']) ?>?refid=${row.trandetailid}" data-bs-toggle="modal" data-bs-target="#modal_list_track"><i class="fa-solid fa-list"></i></a>`;
                                }
                            }
                        ],
                        order: [],
        ...
        selectContact('#filterForm', '#contactSelect', 'employee', 'warehouse', 'Crew');
        ...
        $("#datatable").DataTable({
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    return `
                    <div class="form-check form-check-custom form-check-solid form-check-sm px-3">
                        <input type="checkbox" class="form-check-input row-checkbox select-checkbox" 
                            value="${row.tranid}">
                    </div>`;
                }
            },
            {
                render: function (data, type, row) {
                    ...
                    let printLink = '';
                    if (row.trantype === 'purchase/delivery' && row.reftype === 'sales/order') {
                        printLink = `
                        <li>
                            <a class="dropdown-item text-hover-primary"
                                href="<?= Url::to(['/report/print']) ?>?id=${row.refid}&module=${rowModule}&type=${rowType}" 
                        ...
                        <ul class="dropdown-menu px-2">
                            <li>
                                    <a class="dropdown-item text-hover-info"
                                        href="<?= Url::to(['updatedeli', 'module' => $module, 'type' => $type, 'type_id' => $type_id]) ?>&id=${row.tranid}"
                            ...
                            <li>
                                <button class="dropdown-item text-hover-danger delete-tran" data-id="${row.tranid}">
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    let reftype = row.reftype;

                    return `
                     <button class="btn btn-sm btn-light-primary btn-icon neo-orba"
                            data-id="${row.tranid}" data-reftype="${reftype}">
            ...
            },
            {
                ...
                className: "text-center",
                render: function (data, type, row) {
                    let totalScan = parseFloat(row.count_in || 0);
                    let totalHarusReturn = parseFloat(row.amount_del_refid || 0);
                    let totalBalik = parseFloat(row.item_count || 0);

                    let sisaReturn = totalHarusReturn - totalBalik;
                    let sisa = totalHarusReturn - totalScan;
                    let finish = row.statuspro == '15';

                    let totalPd = parseFloat(row.td_amount || 0);
                    let totalPo = 0;
                    let totalScanPd = 0;

                    if (row.reftype === 'purchase/request' && row.eventtype === '0') {
                        totalScanPd = parseFloat(row.count_pd || 0);
                        totalPo = parseFloat(row.td_amount || 0);
                    } else if (row.reftype === 'purchase/request' && row.eventtype === '1') {
                        totalScanPd = parseFloat(row.count_in_pinjam || 0);
                        totalPo = parseFloat(row.td_amount || 0);
                    } else if (row.reftype === 'purchase/order' && (row.eventtype === '' || row.eventtype === null)) {
                        totalScanPd = parseFloat(row.count_in_pinjam || 0);
                        totalPo = parseFloat(row.amount_po || 0);
                    }

                    let totalScanMissing = parseFloat(row.count_missing || 0);
                    ...
                    if (row.reftype === 'sales/order') {
                        if (finish && totalScan < sisaReturn) {
                            badgeClass = 'bg-light-info text-info';
                            badgeText = `<?= Yii::$app->lang->t('extra', 'complete') ?> (${totalScan}/${totalHarusReturn})`;
                            if (totalScanMissing > 0) {
                                badgeText3 = `Barang Hilang: ${totalScanMissing}`;
                            } else {
                                badgeText3 = '';
                            }
                        } else if (totalScan === 0 && sisaReturn > 0) {
                            badgeClass = 'bg-light-danger text-danger';
                            badgeText = `<?= Yii::$app->lang->t('tran', 'notstarted') ?> (${sisaReturn} items) `;
                            if (totalScanMissing > 0) {
                                badgeText3 = `Barang Hilang: ${totalScanMissing}`;
                            } else {
                                badgeText3 = '';
                            }
                        } else if (totalScan >= sisaReturn && sisaReturn > 0) {
                        ...
                        } else if (totalScan < sisaReturn) {
                            badgeClass = 'bg-light-warning text-warning';
                            badgeText = `Partial (${totalScan}/${totalHarusReturn})`;
                            if (totalScanMissing > 0) {
                                badgeText3 = `Barang Hilang: ${totalScanMissing}`;
                            } else {
                                badgeText3 = '';
                        ...
                        if (row.status === 5) {
                            badgeClass = 'bg-light-danger text-danger';
                            badgeText = 'Cancelled';
                        } else if (row.po_contact === null || row.po_contact === '-') {
                            badgeClass = 'bg-light-warning text-warning';
                            badgeText = 'Reference Not Found';
                        } else if (row.status === 0) {
                            badgeClass = 'bg-light-info text-info';
                            badgeText = `Pending Approval (${totalPo} items)`;
                        } else if (totalScanPd === 0) {
                            badgeClass = 'bg-light-primary text-primary';
                            badgeText = `Not Started (${totalScanPd}/${totalPo})`;
                        } else if (totalScanPd >= totalPo) {
                            badgeClass = 'bg-light-success text-success';
                            badgeText = `<?= Yii::$app->lang->t('extra', 'complete') ?> (${totalScanPd}/${totalPo})`;
                        } else {
                            badgeClass = 'bg-light-warning text-warning';
                            badgeText = `Partial (${totalScanPd}/${totalPo})`;
                        }
                    }

                    return `<span class="badge ${badgeClass} p-2">${badgeText}</span>
                              <small class="text-muted mt-1 mb-0 d-block" style="font-size: 0.75em;">${badgeText3}</small>
                         `;
                }
            },
                <?php if ("$module/$type" == "purchase/delivery" && $type_id == 1) { ?>
                                                                                                                                                       {
                    data: "statuspro",
                    ...
                    render: function (data, type, row, meta) {
                        ...
                        let totalScanRet = parseFloat(row.count_in || 0);
                        let totalScanRetrun = parseFloat(row.item_count || 0);
                        let totalHarusReturn = parseFloat(row.td_amount || 0) + parseFloat(row.amount_ret || 0);
                        let sisa = totalHarusReturn - (totalScanRet + totalScanRetrun);
                        ...
                        if (statusInfo[data]) {
                            ...
                            const trParent = $(`#datatable tbody tr`).filter(function () {
                                return $(this).find(`.neo-orba[data-id="${row.tranid}"]`).length > 0;
                            });

                            const barcodes = trParent.data('pending-barcodes') || "";

                            if (row.reftype === 'sales/order' && data == 10 && row.status == 1 && totalScanRet > 0) {
                                statusOptions = `
                                <li>
                                    <a class="dropdown-item items update-statuspro d-flex align-items-center" 
                                    href="#" data-id="${row.tranid}" 
                                    data-statuspro="15">
                                        <i class="fas fa-check-double text-dark me-2"></i> Finish
                                    </a>
                                </li>
                                `;
                            }
            ...
            },
            {
                ...
                className: "text-start",
                render: function (data, type, row) {
                    let isPo = row.reftype === 'purchase/order' || row.reftype === 'purchase/request';
                    let isIn = row.trantype === 'purchase/delivery';

                    let contact_namedel = '';
                    let jobcompanydel = '';
                    if (isIn && !isPo) {
                        contact_namedel = row.ref_contact_name || '-';
                        jobcompanydel = row.ref_jobcompany || '-';
                    } else if (isPo) {
                        contact_namedel = row.po_contact || '-';
                        jobcompanydel = row.po_jobcompany || '-';
                    } else {
                        contact_namedel = row.ref_contact_name || '-';
                        jobcompanydel = row.ref_jobcompany || '-';
                    }
            ...
            },
            {
                ...
                <?php if ("$module/$type" == "purchase/delivery" && $type_id == 2) { ?>
                                                                                                                        {
                    data: "eventtype",
                    className: "text-start",
                    render: function (data) {
        ...
        $(document).on('click', '.detail-tran', function () {
            ...
            var baseUrl = '<?= Url::to(['updatedeli', 'module' => $module, 'type' => $type, 'type_id' => $type_id]) ?>';
            var fullUrl = baseUrl + (baseUrl.indexOf('?') !== -1 ? '&' : '?') + 'id=' + id;
            ...
            $.ajax({
                ...
                error: function () {
                    $('#modal-content').html('<p class="text-danger p-3">Error loading form detail.</p>');
                    $('#modal_form_tran').modal('show');
                }
            });
        });
