...
if ("$module/$type" == "sales/delivery") {
    $urladd = Url::to(['createdeli', 'module' => $module, 'type' => $type]);
    $this->title = Yii::$app->lang->t('extrasidebar', 'extrasidebar16');
    $formatnumber = "D";
} elseif ("$module/$type" == "purchase/delivery") {
    $urladd = Url::to(['createdeli', 'module' => $module, 'type' => $type]);
        ...
        <table class="table align-left table-row-dashed fs-6 gy-5" id="datatable">
            <thead>
                <tr class="text-start bg-gray-100 fs-6 text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                    ...
                    <th class="text-center min-w-150px">Status Stock Out</th>
                    ...
                    <th class="text-start min-w-150px">
                        <?= Yii::$app->lang->t('extra', 'delivery5') ?>
                    </th>
    ...
    $(document).ready(function () {
        ...
        function addNestedDropdown() {
            $(document).on('click', '.neo-orba', function (e) {
                                ...
                                <table class="table align-middle table-row-dashed fs-6 gy-5 sub-datatable"
                                    ...
                                    <thead>
                                        <tr class="text-gray-500 bg-gray-100 border-bottom border-gray-200 fs-7 text-uppercase">
                                            ...
                                            <th class="text-center">Send Milik</th>
                                            <th class="text-center">Send Pinjam</th>
                ...
                var subTable = $(`#sub-table-${parentId}`).DataTable({
                    processing: true,
                    serverSide: true,
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            return `
                                <a class="btn btn-sm btn-variant-deli-out"
        ...
        $("#datatable").DataTable({
            ...
            },
            {
                ...
                orderable: false,
                render: function (data, type, row) {
                    const hasReturn = parseFloat(row.total_ret_docs) > 0 && row.trantype === 'sales/delivery';
                    const isDisabled = hasReturn ? 'disabled' : '';

                    return `
                    <div class="form-check form-check-custom form-check-solid form-check-sm px-3">
                        <input type="checkbox" 
                            class="form-check-input row-checkbox select-checkbox" 
                            value="${row.tranid}" 
                            ${isDisabled} 
                            ${hasReturn ? 'title="Data ini tidak bisa diproses karena sudah ada Return"' : ''}>
                    </div>`;
                }
            },
            {
                render: function (data, type, row) {
                    ...
                    const canDelete = parseFloat(row.total_ret_docs) <= 0;
                            ...
                            <ul class="dropdown-menu px-2">
                                <li>
                                    <a class="dropdown-item text-hover-info"
                                        href="<?= Url::to(['updatedeli', 'module' => $module, 'type' => $type]) ?>&id=${row.tranid}"
                                        data-bs-toggle="modal"
                                        data-bs-target="#modal_form_tran">
                                ...
                                <li>
                                    <a class="dropdown-item text-hover-primary"
                                        href="<?= Url::to(['/report/print']) ?>?id=${row.tranid}&module=${rowModule}&type=${rowType}"
            ...
            },
            {
                ...
                className: "text-center",
                render: function (data, type, row) {
                    let totalScan = row.total_scan_del || 0;
                    let totalDel = row.amount_del || 0;
                    let sisa = totalDel - totalScan;
                    let status = row.status;

                    let badgeClass = 'bg-secondary';
                    let badgeText = 'Unknown';
                    let text = totalScan < totalDel ? `<?= (Yii::$app->lang->t('tran', 'completescan')) ?> ${sisa} items` : '';

                    if (totalScan === 0 && status === 0) {
                        badgeClass = 'bg-light-danger text-danger';
                        badgeText = `<?= (Yii::$app->lang->t('tran', 'notstarted')) ?>, Approve Stock Out`;
                    } else if (totalScan === 0) {
                        badgeClass = 'bg-light-danger text-danger';
                        badgeText = `<?= (Yii::$app->lang->t('tran', 'notstarted')) ?> (${totalDel} items)`;

                    } else if (totalScan < totalDel) {
                        badgeClass = 'bg-light-warning text-warning';
                        badgeText = `Partial (${totalScan}/${totalDel})`;

                    } else if (totalScan >= totalDel && totalDel > 0) {
                        badgeClass = 'bg-light-success text-success';
                        badgeText = `<?= (Yii::$app->lang->t('extra', 'complete')) ?>  (${totalScan}/${totalDel})`;
                    } else if (status === 5) {
                        badgeClass = 'bg-light-danger text-danger';
                        badgeText = `Cancelled`;
                    }
                    ...
                    return `
                        <div class="d-flex flex-column align-items-center mx-auto my-auto w-100">
                            ...
                            <p class="text-muted mt-1 mb-0" style="font-size: 0.75em;">
                            ${text}
                            </p>
                        </div>`;
                }
                ...
                render: function (data, type, row) {
                    ...
                    if (statusInfo[data]) {
                        ...
                        if (data == 0) {
                            ...
                            `;
                        } else if (data == 1 && parseFloat(row.total_ret_docs) <= 0) {
                            statusOptions = `
            ...
            },
            {
                ...
                className: "text-start",
                render: function (data, type, row) {
                    let isPo = row.reftype === 'purchase/order';
