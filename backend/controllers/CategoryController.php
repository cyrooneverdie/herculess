...
use yii\data\AcytiveDataProvider;
...
class CategoryController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        $model->setAttribute('categorycode', $nextCode);
    ...
    public function actionAddkategori()
    {
        ...
        try {
            ...
            if (!empty($code)) {
                ...
                // Generate code
                $model->categorycode = $model->nextKode();
            ...
            if ($model->validate() && $model->save()) {
                ...
                Yii::error('Error saving category: ' . json_encode($model->errors), 'categorys');
            ...
            Yii::error('Exception when saving category: ' . $e->getMessage(), 'categorys');
    ...
    public function actionEditkategori()
    {
        ...
        try {
            ...
            if ($model->validate() && $model->save()) {
                ...
                // Log the errors for debugging
                Yii::error('Error updating category: ' . json_encode($model->errors), 'categorys');
            ...
            // Log the exception for debugging
            Yii::error('Exception when updating category: ' . $e->getMessage(), 'categorys');
    ...
    public function actionList()
    {
        ...
        $ordercolumn = $params['columns'][$sortcolumn]['data'] ?? 'categorycode';
        $columnorder = $params['order'][0]['dir'] ?? 'ASC';

        // Validasi sorting biar aman dari SQL Injection
        $allowedColumns = ['categorycode', 'categoryname'];
        if (!in_array($ordercolumn, $allowedColumns)) {
            $ordercolumn = 'categorycode'; // Default sorting
        }
        ...
        if (isset($search) && $search !== "") {
            $query .= " AND (
                k.categoryname ILIKE '%$search%' OR
                k.categorycode ILIKE '%$search%'
            )";
        }
    ...
    {
        try {
            $nextKode = (new Category())->nextKode();
